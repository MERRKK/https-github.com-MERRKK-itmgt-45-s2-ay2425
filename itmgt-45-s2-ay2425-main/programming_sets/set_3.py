...
def relationship_status(from_member, to_member, social_graph):
    ...
    # Check if from_member comes from to_member (assume it is false first)
    ...
    # Check if to_member comes from from_member (assume it is false first)
    to_follows_from = False
    if to_member in social_graph and "following" in social_graph[to_member]:
        if from_member in social_graph[to_member]["following"]:
            to_follows_from = True
    
    # Determine relationship status based on who follows whom
...
# Stay within the function. Only use the parameters as input. The function should return your answer.
    ...
    # If starting and ending at the same stop, one whole cycle of route will be conducted!
    ...
    # Otherwise, we need to find the path from first_stop to second_stop
    current_stop = first_stop
    total_time = 0
    
    # Create a direct lookup for the next stop and travel time
    next_stops = {}
    for leg in route_map:
        start, end = leg
        next_stops[start] = {
    ...
    # Travel along the route until we reach the second stop
